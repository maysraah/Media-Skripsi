...
<div class="container d-flex justify-content-center align-items-center center-container px-2">
    <div class="card shadow w-100" style="max-width: 600px;">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0">Instruksi Evaluasi</h5>
        </div>
        <div class="card-body">
            @if ($sudahMengerjakan && $lulus)
                <p class="fs-6 text-success">✅ Anda sudah mengerjakan evaluasi ini dan mendapatkan skor <strong>{{ $nilai->skor }}</strong>.</p>
                <p class="fs-6">Karena skor Anda sudah memenuhi KKM (70), Anda <strong>tidak perlu mengerjakan ulang</strong> evaluasi ini.</p>
            @else
        ...
        <div class="card-footer d-flex justify-content-end gap-2">
            @if (!($sudahMengerjakan && $lulus))
                ...
                <a href="{{ route('siswa.materi', ['bab' => 4, 'page' => 4]) }}" class="btn btn-danger">Batal</a>
                <a href="{{ route('kuis.evaluasi') }}" class="btn btn-primary" id="modalKuisBtn">Kerjakan Evaluasi</a>
            @else
